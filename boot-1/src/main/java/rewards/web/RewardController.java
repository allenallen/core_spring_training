...
public class RewardController {
	...
	public String processReward(@RequestParam("purchaseAmount") String amount,
			@RequestParam("merchantNumber") String merchantNumber, @RequestParam("cardNumber") String cardNumber,
			Model model) {
		System.out.println(amount + "," + merchantNumber + "," + cardNumber);
		Purchase purchase = new Purchase(monetaryAmountOf(amount), merchantNumber, cardNumber);
		model.addAttribute("rewardConfirmation", rewardsService.rewardAccountFor(purchase));
		return "reward";
	}
