...
// : Refactor this repository to use Spring's JdbcTemplate and a ResultSetExtractor called AccountExtractor.
...
public class JdbcAccountRepository implements AccountRepository {
	...
	private static final String SQL_FINDBY_CARDNUMBER = "SELECT a.ID AS ID, a.NUMBER AS NUMBER, a.NAME AS NAME, a.TOTAL_POINTS AS TOTAL_POINTS,"
			+ " b.NUMBER AS CARD_NUMBER, b.ID AS CARD_ID" + " FROM T_ACCOUNT a, T_ACCOUNT_CARD c"
	...
	public Account findByCardNumber(String cardNumber) {
		// return jdbcTemplate.query(SQL_FINDBY_CARDNUMBER, new AccountExtractor(),
		// cardNumber);
		return jdbcTemplate.query(SQL_FINDBY_CARDNUMBER, (rs) -> {
			return mapAccount(rs);
		}, cardNumber);
		// try {
	...
	private Card mapCard(ResultSet rs) throws SQLException {
		 ...
		 * ReflectionUtils.makeAccessible(idField); ReflectionUtils.setField( idField,
		 * card, rs.getLong("CARD_ID"));
		 */
		return card;
	}

	private static final String SQL_UPDATE_ACCOUNT = "UPDATE T_ACCOUNT a" + " SET a.TOTAL_POINTS = ?"
			+ " WHERE a.NUMBER = ?";

	@Override
	public void updateAccount(Account account) {
		int rowsUpdated = jdbcTemplate.update(SQL_UPDATE_ACCOUNT, account.getTotalPoints(), account.getNumber());

		if (rowsUpdated != 1) {
			throw new IncorrectResultSizeDataAccessException(1, rowsUpdated);
		}
		// try {
		// Connection conn = dataSource.getConnection();
		// try {
